...
`;
  ...
  function extractVideoTitle() {
    return document.querySelector('h1.ytd-watch-metadata yt-formatted-string, meta[name="title"]')?.textContent?.trim() || document.querySelector('meta[property="og:title"]')?.content || document.title.replace(" - YouTube", "");
  }
  function extractChannelName() {
    return document.querySelector('#channel-name a, ytd-channel-name a, [itemprop="author"] [itemprop="name"]')?.textContent?.trim() || document.querySelector('meta[itemprop="name"]')?.content || "";
  }
  ...
  async function extractTranscript() {
    const transcriptSegments = document.querySelectorAll(
      "ytd-transcript-segment-renderer, ytd-transcript-body-renderer .segment, .ytd-transcript-segment-renderer"
    );
            ...
            if (tracks.length > 0) {
              ...
              if (englishTrack?.baseUrl) {
                ...
                textNodes.forEach((node) => {
                  ...
                  const decoded = node.textContent.replace(/&#39;/g, "'").replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&lt;/g, "<").replace(/&gt;/g, ">");
  ...
  function buildVideoContent() {
    ...
    if (meta.isLive) html += `<p><strong>\u{1F534} Live Stream</strong></p>`;
